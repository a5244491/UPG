...
/**
 * 
 * 功能名称 ： 支付Action -前台模式 实现目标 ： 业务系统请求此Action完成后台支付操作 入口数据 ：
 * 业务系统的请求数据包data_package 出口数据 ： 支付结果页面URL（由对应Action处理跳转）-success.jsp 支付成功
 * -failure.jsp 支付失败
 * ------------------------------------------------------------
 * --------------------------- 数据流程 ： 1 获取业务端/客户端请求数据包（data_package） 2
 * 解析data_package a 获取消息头data_package_header b 获取消息体data_package_body 3
 * 使用data_package_header中的客户端ID与服务ID验证客户端使用权限 4 权限验证通过
 * 使用data_package_header中的signData验证客户端数据签名 5 签名验证通过 a 生成支付接口协议报文 b
 * 向支付记录表中新增一条支付记录（状态为 '待支付'） c 使用服务携带的支付通道ACCEPT_PAY_ID获取支付接口接入信息 1
 * 调用支付接口信息中的协议转化类实现对生成的支付接口协议报文,本身的支付信息进行转化为支付接口的请求数据包 d
 * 使用对应的支付接口完成支付操作（调用支付合作平台的支付接口完成支付操作） e
 * 使用服务携带的PAY_RESULT_PROCESS_CLASS完成支付结果处理操作 f 在当前事务中,对新增的支付记录信息进行更新操作（状态为
 * '支付成功'） 6 返回支付URL 'success.jsp'-返回支付成功页面（支付网关前台提供）
...
@Namespace("/test")
@Action(value = "payFService", results = { @Result(name = "success", location = "/pay/goPay.jsp"),
		@Result(name = "failure", location = "/pay/failure.jsp") })
public class PayNetServiceFModeAction extends BaseAction {
	protected final Log log = LogFactory.getLog(PayNetServiceFModeAction.class);
	...
	// (CacheService)getService("cacheService");
	...
	public String service() throws Exception {
		...
		try {
			...
			if (clientDomain == null) {
				log.debug("F请求支付__未取得接入商户信息或商户信息已失效");
			...
			if (!verifyOPSNResult) {
				log.debug("F请求支付__操作流水号格式不正确（无效操作流水号）");
			...
			if (clientService.checkClientRight(clientid, serviceid) == 1) {
				...
				if  (MessageUtil.verifyFPaySign(paramMap,reqHeadMsg.getSign_data(),clientPubKey))
					...
					// 验证报文签名失败
					log.debug("F请求支付__验证报文签名失败");
					request.setAttribute(IConstants.FPAY_EEROR_MSG, "your sign_data is worng.");
					return "failure";
				}
			} else {
				// 验证客户权限失败
				log.debug("F请求支付__验证客户权限失败");
				request.setAttribute(IConstants.FPAY_EEROR_MSG, "auth faild,you have no right to use this service.");
				return "failure";
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			log.debug("F请求支付__异常" +  ex.getMessage());
			request.setAttribute(IConstants.FPAY_EEROR_MSG, "the system exception now at " + new java.util.Date()
